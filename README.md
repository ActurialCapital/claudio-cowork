...
├── AGENTS.md                             ← Cross-tool project context (if configured)
├── CLAUDE.md                             ← Claude-specific behavior rules (if configured)
├── your existing files...
├── claudio-cowork/                       ← Stays local, git-ignored
│   ├── Makefile
│   ├── INSTRUCTIONS/                     ← Templates for AGENTS.md and CLAUDE.md
│   ├── SKILLS/
│   ├── scripts/
│   └── ...
...
Each configuration module can also run independently: `make about-me`, `make code-style`, `make feedback`, `make global-instructions`, `make instructions` (or individually: `make agents-md`, `make claude-md`).
...
For complex projects that need multi-agent coordination, `make plugins` installs the recommended agent stack: **GSD** (orchestration) + **Superpowers** (quality enforcement). Instruction files (**AGENTS.md** and **CLAUDE.md**) are configured separately via `make instructions` — they are project configuration files, not plugins.
...
| Behavior rules | CLAUDE.md | Think-before-coding, simplicity, surgical changes |
| Mistake tracking | CLAUDE.md | Dated correction log, applied as overrides |
...
```mermaid
flowchart TB
    CC["Claudio-Cowork\n(context)"] -->|"persistent memory,\nprofile, writing rules"| INST
    subgraph INST["Instruction Files"]
        AM["AGENTS.md\n(project context)"]
        CM["CLAUDE.md\n(behavior rules)"]
    end
    INST -->|"build, test, lint,\nconventions, behavior"| GSD
...
GSD handles orchestration: how to break a large objective into a dependency graph of tasks, execute them in parallel waves, and track cost. Superpowers handles discipline: how to ensure each task follows TDD, gets reviewed, and meets quality standards. AGENTS.md handles project context: build commands, test runners, coding conventions, and boundaries — shared across all AI coding agents (Codex, Cursor, Copilot, Gemini CLI, not just Claude). CLAUDE.md handles Claude-specific behavior: how to approach problems, simplicity constraints, surgical change rules, and accumulated mistake corrections. Claudio-Cowork handles personal context: who you are, how you write, and what conventions your project follows.
...
9. **Install plugins** — **1 (Yes)** or **2 (No)**. Installs GSD + Superpowers. Run `make plugins` later if skipped.
10. **Configure instruction files** — **1 (Yes)** or **2 (No)**. Configures `AGENTS.md` (cross-tool project context) and `CLAUDE.md` (Claude-specific behavior rules). Each file offers its own Context/Default/Customize/Skip flow. Run `make instructions` later if skipped.
